...
open class Configurator(jsonConfigFile: RootCfg) {

    private val jsonConfigFileLocal = jsonConfigFile

    // Возвращает описание класса по его коду
    // Если описание класса в конфигурации не найдено, то вернёт null
    open fun getClassDescriptionByCode(
        codeClass: String
    ): ObjectCfg? {
...
// класс используется для работы со шкалами
class Scalable(jsonConfigFile: RootCfg) : Configurator(jsonConfigFile) {
    ...
    public override fun getClassDescriptionByCode(
        codeClass: String
    ): ObjectCfg {

        var configClass: ObjectCfg? = null
        configClass = super.getClassDescriptionByCode(codeClass)
    ...
    // Возвращает название класса, на который есть ссылка в структуре scale в файле конфигурации
    // Если такого класса нет в конфигурации, то функция вернет ""
    public fun getClassNameByScaleKeyType(
        keyType: String
    ): String {
    ...
    // Возвращает описание класса шкалы в файле конфигурации. Поиск идет по ключу "keyType": "InScale" на класс шкалы.
    ...
    // Возвращает название поля с идентификатором шкалы по классу из объекта и ключам keyType == "InScale" или "InParentScale"
    ...
    // Возвращает название поля с идентификатором шкалы по описанию класса и ключам keyType == "InScale" или "InParentScale"
    // Если поле не найдено, то вернет ""
    ...
    // Возвращает название класса(scaleComponent) описывающего компонент шкалы тарифа в файле конфигурации.
    ...
    // Возвращает название класса(scaleComponentValue) описывающего связь компонента шкалы тарифа и измеряемой величины тарифа в файле конфигурации.
    ...
    // Возвращает название класса(scalableAmount) описывающего измеряемую величину тарифа в файле конфигурации.
    ...
    // Возвращает название поля по ключу "keyType" из ссылки в структуре scale
    // Если поле не найдено, то вернет ""
